...
def get_word_score(word, n):
    ...
    second=7*len(word_lower)-3*(n-len(word_lower))
    total=first*max(1,second)
    return total
...
def update_hand(hand, word):
    ...
    new_hand = {}
    for letter in hand.keys():
        new_hand[letter] = max(0, hand[letter] - word_dict.get(letter,0))
    return new_hand
...
def calculate_handlen(hand):
    ...
    returns: integer
    """
    return  sum(hand.values())  # TO DO... Remove this line when you implement this function
    ...
    while sum(hand.values()) > 0:
        ...
        if word == '!!':
            ...
            if is_valid_word(word, hand, word_list) == True:     
                ...
                print(f'{word} earned {get_word_score(word, n)} points. Total: {total_score} points')
            else:
...
def substitute_hand(hand, letter):
    ...
    if letter not in hand:
        ...
        remaining_alphabet = list(string.ascii_lowercase)
        for key in hand.keys():
            if key != '*':
                remaining_alphabet.remove(key)
        num = hand.pop(letter)
        new_letter = random.choice(remaining_alphabet)
        hand[new_letter] = num
    return hand

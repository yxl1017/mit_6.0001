...
class SubMessage(object):
    ...
    def build_transpose_dict(self, vowels_permutation):
        ...
        '''    
        li = list(string.ascii_uppercase) + list(string.ascii_lowercase)
        transpose_dict = {}
        for ele in li:
            transpose_dict[ele] = ele
            
        upper_permutation = vowels_permutation.upper()
        for letter, i in zip(VOWELS_UPPER, range(len(upper_permutation))):
            transpose_dict[letter] = upper_permutation[i]
        for letter, i in zip(VOWELS_LOWER, range(len(vowels_permutation))):
    ...
    def apply_transpose(self, transpose_dict):
        ...
        for char in self.get_message_text():
            if char in VOWELS_UPPER + VOWELS_LOWER:
                new_char = transpose_dict[char]
            else:
                new_char = char
            transposed_text += new_char
...
class EncryptedSubMessage(SubMessage):
    ...
    def decrypt_message(self):
        ...
        '''
        vowels_permutation_list = get_permutations('aeiou')
        max_validwords = 0
